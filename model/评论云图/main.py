...
def render_cloud(word_counts_top100):
    ...
    word1.set_global_opts(title_opts=opts.TitleOpts('云图'),
                          ...
                          tooltip_opts=opts.TooltipOpts(is_show=True, background_color='white', border_color='yellow'))
    # 渲染在html页面上
    word1.render("差评云图.html") # 更换数据集修改此处

if __name__ == '__main__':
    # 读取文件
    with open('poor.txt', encoding='utf-8') as f:# 更换数据集修改此处
        data = f.read()
    ...
    for i in range(0,100):
            print(word_counts_top100[i],end='')
            if(i%7==6):print("\n")
    # 渲染词云
